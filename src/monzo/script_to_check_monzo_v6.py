...
class MonzoExpenseTracker:
    ...
    def fetch_transactions(self, days_back=90):
        ...
        print(f"Account ID: {active_account.identifier}")
        
        # Debug: Print account info
        for attr in dir(active_account):
            if not attr.startswith('_') and not callable(getattr(active_account, attr)):
                try:
                    print(f"Account.{attr} = {getattr(active_account, attr)}")
                except:
                    print(f"Account.{attr} = <error accessing>")
        
        try:
            # Fetch all transactions first
            print("Fetching all available transactions...")
            transactions = self.client.transactions(account_id=active_account.identifier)
